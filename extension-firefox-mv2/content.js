(() => {
  ...
  const stopUIPresent = () =>
    !!document.querySelector('button[aria-label="Stop generating"],[data-testid="stop-button"]');
  ...
  const tick = () => {
    ...
    if (grew && !state.active) { state.active = true; startBeep(); }
    if (grew) state.lastChange = now();
    const idleMs = now() - state.lastChange;
    if (state.active && idleMs > 1000 && !stopUIPresent()) { state.active = false; doneBeep(); }
    state.lastLen = len;
  };
  ...
  const prime = () => {
    const h = () => { ctx.resume(); document.removeEventListener('pointerdown', h, true); };
